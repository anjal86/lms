...
const marker = psql(['-tAc', "select coalesce(position('service_levels.edit' in pg_get_functiondef(to_regprocedure('public.has_workspace_permission(text)'))),0) > 0"]).trim();
if (marker === 't') {
  console.log('  ✓ 202609120038_service_level_permissions.sql');
  process.exit(0);
}

console.log('  → Applying 202609120038_service_level_permissions.sql');
try {
  psql(['-f', '/app-migrations/202609120038_service_level_permissions.sql'], 'inherit');
  psql(['-c', "NOTIFY pgrst, 'reload schema';"], 'inherit');
  console.log('  ✓ 202609120038_service_level_permissions.sql');
} catch {
  console.error('Migration failed: 202609120038_service_level_permissions.sql');
  process.exit(1);
}
