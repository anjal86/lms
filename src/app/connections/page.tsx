...
export default function ConnectionsPage() {
  ...
  // Open WhatsApp drawer automatically if requested in URL
  useEffect(() => {
    ...
    return (
      <div className="empty-state surface-flat">
        ...
        <p className="empty-state-description">
          Agents can use connected channels from lead workspaces without seeing account credentials.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="app-page space-y-6">
      {/* Clean Linear/Attio-grade Header */}
      <header className="flex flex-col gap-4 border-b border-zinc-200/80 pb-5 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1.5 max-w-2xl text-sm leading-6 text-zinc-500">
            Connect Facebook, Instagram, WhatsApp, TikTok, email, and webhooks to feed your unified CRM inbox automatically.
          </p>
        </div>
      ...
      {connectedProvider && (
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">
          <div className="flex items-center gap-2 font-semibold">
            <CheckCircle2 className="h-4 w-4 text-emerald-600" /> {connectedProvider[0]?.toUpperCase()}{connectedProvider.slice(1)} connected
          </div>
          <p className="mt-1 text-xs text-emerald-700">New events from this account will now enter the lead pipeline.</p>
        </div>
      )}
      ...
      {(error || data.migrationRequired) && (
        <div role="alert" className="rounded-lg border border-amber-200 bg-amber-50 p-4">
          <div className="flex items-center gap-2 text-sm font-semibold text-amber-800">
            <AlertCircle className="h-4 w-4" /> Setup needs attention
          </div>
          <p className="mt-1 text-xs leading-5 text-amber-700">{error || data.message}</p>
          {data.migrationRequired && (
            <code className="mt-2 inline-block rounded bg-white/70 px-2 py-1 text-[11px] text-amber-900">
      ...
      {/* Grid of Lead Channels */}
      <section>
        <div className="mb-3.5">
          <h2 className="text-sm font-semibold text-zinc-950">Active Lead Sources</h2>
          <p className="text-xs text-zinc-500">
            Inbound messages and forms create or match contacts and route them according to your workflow rules.
          </p>
        </div>
        ...
        <div className="grid gap-3.5 sm:grid-cols-2 lg:grid-cols-3">
          {data.catalog.map((provider) => {
            ...
            // WhatsApp specific status calculation
            ...
            const providerConnected = provider.id === 'whatsapp'
              ? (active?.status === 'connected' || baileysConnectedCount > 0)
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex min-w-0 items-start gap-3">
                      <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border ${ACCENTS[provider.id]}`}>
                        <Icon className="h-5 w-5" />
                      </span>
                      <div className="min-w-0">
                        <h3 className="text-sm font-semibold text-zinc-950">{provider.name}</h3>
                        <p className="mt-0.5 line-clamp-2 text-xs leading-5 text-zinc-500">{provider.description}</p>
                      </div>
                    </div>
                    <span className="inline-flex shrink-0 items-center gap-1.5 text-[11px] font-medium text-zinc-600">
                      ...
                      />
                      {providerConnected
                        ? (provider.id === 'whatsapp' && baileysConnectedCount > 1 ? `${baileysConnectedCount} connected` : 'Connected')
                        : providerPaused
                        ? 'Paused'
                        : providerPending
                        ? 'Scan needed'
                  ...
                  {/* WhatsApp Custom Compact Summary */}
                  {provider.id === 'whatsapp' && (
                    <div className="mt-3.5 space-y-1.5">
                      {baileysConnections.length > 0 ? (
                        <div className="divide-y divide-zinc-100 rounded-md border border-zinc-200 bg-zinc-50/50">
                          {baileysConnections.slice(0, 3).map((conn) => {
                            const isConn = conn.status === 'connected';
                            const phone = conn.external_account_id;
                            return (
                              <div key={conn.id} className="flex items-center justify-between gap-2 px-3 py-2 text-xs">
                                <div className="min-w-0">
                                  <div className="truncate font-semibold text-zinc-900">{conn.display_name || 'WhatsApp'}</div>
                                  {phone && <div className="font-mono text-[10px] text-zinc-500">+{phone}</div>}
                                </div>
                                <span className="inline-flex shrink-0 items-center gap-1 text-[10px] font-medium text-zinc-500">
                                  <span className={`h-1.5 w-1.5 rounded-full ${isConn ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                                  {isConn ? 'Active' : 'Scan QR'}
                                </span>
                              </div>
                            );
                          })}
                          {baileysConnections.length > 3 && (
                            <div className="px-3 py-1 text-[10px] text-zinc-500">
                              +{baileysConnections.length - 3} more account{baileysConnections.length - 3 > 1 ? 's' : ''}
                            </div>
                          )}
                        </div>
                      ) : (
                        <div className="rounded-md border border-dashed border-zinc-200 bg-zinc-50/40 p-3 text-center text-xs text-zinc-500">
                          No linked devices yet. Click below to pair via QR code.
                        </div>
                      )}
                    </div>
                  )}

                  {/* Standard active connection summary for non-WhatsApp channels */}
                  {provider.id !== 'whatsapp' && active && (
                    <div className="mt-3.5 rounded-md border border-zinc-200 bg-zinc-50/60 p-2.5 text-xs">
                      ...
                      <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-[10px] text-zinc-500">
                        {active.last_event_at && (
                          <span>Last event {new Date(active.last_event_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                        )}
                        {active.external_account_id && <span className="font-mono">{active.external_account_id}</span>}
                      </div>
                      {active.last_error && <div className="mt-1.5 text-[10px] text-red-600">{active.last_error}</div>}
                    </div>
                  )}
                </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 border-t border-zinc-100 bg-zinc-50/70 px-4 py-2.5">
                    ...
                    {!provider.configured && (
                      <button
                        ...
                        className="button-ghost button-sm text-zinc-500"
                      >
                  ...
                  <div className="flex flex-wrap items-center gap-2 border-t border-zinc-100 bg-zinc-50/70 px-4 py-2.5">
                    {!active || active.status === 'disconnected' ? (
                      provider.connectMode === 'manual' ? (
                        <button
                          type="button"
                          onClick={() => void connectManual(provider)}
                          disabled={isBusy || data.migrationRequired}
                          className="button-primary button-sm"
                        >
                        ...
                        <button
                          ...
                          onClick={() => setSetupProviderId(provider.id)}
                          className="button-primary button-sm"
                          disabled={data.migrationRequired}
                        >
                      ...
                      <button
                        type="button"
                        onClick={() => void updateConnection(active, 'resume')}
                        disabled={isBusy}
                        className="button-primary button-sm"
                      >
                        <PlugZap className="h-3.5 w-3.5" /> Resume
                      </button>
                    ) : (
                      <button
                        type="button"
                        onClick={() => void updateConnection(active, 'pause')}
                        disabled={isBusy}
                        className="button-secondary button-sm"
                      >
                    ...
                    {active && active.status !== 'disconnected' && (
                      <button
                        type="button"
                        onClick={() => void updateConnection(active, 'disconnect')}
                        disabled={isBusy}
                        className="button-ghost button-sm text-red-600"
                      >
      ...
      <section className="surface-flat overflow-hidden rounded-lg border border-zinc-200">
        <div className="border-b border-zinc-200 bg-zinc-50/50 px-4 py-3">
          <h2 className="text-xs font-semibold text-zinc-950">Provider Portal Webhooks &amp; Redirect URLs</h2>
          <p className="mt-0.5 text-[11px] text-zinc-500">
            Paste these HTTPS endpoints in the Meta App Dashboard, TikTok Developer Portal, or external webhook senders.
          </p>
        </div>
        <div className="divide-y divide-zinc-100">
          {[...oauthCallbackRows, ...webhookRows].map(([label, url]) => {
            ...
            return (
              <div key={label} className="flex flex-col gap-2 px-4 py-2.5 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <div className="text-xs font-medium text-zinc-800">{label}</div>
                  <code className="mt-0.5 block break-all font-mono text-[11px] text-zinc-500">{url}</code>
                </div>
                <button
                  type="button"
      ...
      {/* Slide-over Flyout Drawer: WhatsApp Devices & Multi-Account Manager */}
      <BaileysWhatsAppConnect
        isOpen={whatsappDrawerOpen}
        onClose={() => setWhatsappDrawerOpen(false)}
        onChanged={load}
      />

      {/* Slide-over Flyout Drawer: Official Provider Setup Instructions */}
      {setupProvider && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setSetupProviderId(null);
          }}
        >
          <div className="h-full w-full max-w-[460px] overflow-y-auto border-l border-zinc-200 bg-white shadow-xl">
            <div className="flex items-start justify-between gap-3 border-b border-zinc-200 p-4">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">Setup required</p>
                <h2 id="connection-setup-title" className="text-base font-semibold text-zinc-950">
                  {setupProvider.name}
                </h2>
                <p className="mt-1 text-xs leading-5 text-zinc-500">
                  Add these server-side values to `.env.local`, restart Next.js, then return here to connect.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-5 p-4">
              <section>
                ...
                <div className="mt-2 divide-y divide-zinc-100 rounded-md border border-zinc-200">
                  {setupProvider.setup.missing.length > 0 ? (
                    setupProvider.setup.missing.map((key) => (
                      <div key={key} className="flex items-center justify-between gap-3 px-3 py-2">
                        <code className="break-all font-mono text-[11px] text-zinc-700">{key}</code>
              ...
              <section>
                ...
                <div className="mt-2 rounded-md border border-zinc-200 bg-zinc-50 p-3">
                  {setupProvider.setup.required.map((key) => (
                    <code key={key} className="mb-1 block font-mono text-[11px] text-zinc-700">
                      {key}=
                    </code>
                  ))}
                </div>
              ...
              <section>
                <h3 className="text-xs font-semibold text-zinc-800">
                  {setupProvider.connectMode === 'meta_oauth' ? 'Meta developer portal URLs' : 'TikTok developer portal URLs'}
                </h3>
                <div className="mt-2 divide-y divide-zinc-100 rounded-md border border-zinc-200">
                  {(setupProvider.connectMode === 'meta_oauth'
                    ? [
                        [
                          `${setupProvider.shortName} OAuth callback`,
                          setupUrls?.metaOauthCallbacks[setupProvider.id as 'facebook' | 'instagram' | 'whatsapp'] ||
                            `${appOrigin}/api/integrations/oauth/${setupProvider.id}/callback`,
                        ],
                        ['Meta webhook callback', metaWebhook],
                      ]
                    : [
                        [
                          ...
                          setupUrls?.tiktokOauthCallback || `${appOrigin}/api/integrations/oauth/tiktok/callback`,
                        ],
                        ['TikTok webhook callback', tiktokWebhook],
                      ]
                  ).map(([label, url]) => (
                    <div key={label} className="px-3 py-2">
                      ...
                      <code className="mt-1 block break-all font-mono text-[11px] text-zinc-500">{url}</code>
                    </div>
                  ))}
                </div>
              </section>
