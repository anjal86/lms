    ...
    <div className="grid gap-4 lg:grid-cols-[300px_minmax(0,1fr)]">
      ...
      <section className="surface-flat p-5">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          ...
          <label className="text-xs font-semibold text-zinc-700">Team key<input value={draft.team_key} disabled={Boolean(draft.id)} onChange={(event) => setDraft((current) => ({ ...current, team_key: keyFromName(event.target.value) }))} placeholder="sales" className="field mt-1.5 font-mono disabled:bg-zinc-100 disabled:text-zinc-500" /><span className="mt-1 block text-[10px] font-normal text-zinc-400">{draft.id ? 'Stable after creation because conversations and automations reference this key.' : 'Stable routing identifier used by automations and conversations.'}</span></label>
        </div>
