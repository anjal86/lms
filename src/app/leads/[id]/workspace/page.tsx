...
type WorkspaceTab = 'overview' | 'activity' | 'commercial' | 'operations';
...
const TABS: Array<{ id: WorkspaceTab; label: string; description: string }> = [
  { id: 'overview', label: 'Overview', description: 'What matters now' },
  { id: 'activity', label: 'Activity', description: 'Conversations and notes' },
  { id: 'commercial', label: 'Commercial', description: 'Quote, itinerary and payments' },
  { id: 'operations', label: 'Operations', description: 'People, documents and delivery' },
];
...
export default function LeadWorkspacePage() {
  ...
  useEffect(() => {
    document.title = lead ? `${lead.customer_name} — Opportunity` : 'Opportunity — CRM';
  }, [lead]);
  ...
  if (!lead) {
    return (
      <div className="empty-state surface-flat">
        ...
        <h1 className="empty-state-title mt-3">Opportunity unavailable</h1>
        <p className="empty-state-description">It may have been removed, or your account may not have access to it.</p>
        <button type="button" onClick={() => router.push(currentUser.role === 'agent' ? '/my-work' : '/leads')} className="button-primary mt-4">Back to opportunities</button>
      </div>
    );
  }
  ...
  const showPayments = ['quote_sent', 'in_negotiation', 'won', 'lost'].includes(lead.stage) || packageValue > 0;
  const showDelivery = lead.stage === 'won' || tripStatus !== 'planning' || suppliers.length > 0 || checklistTotal > 0;
  ...
  return (
    <div className="app-page">
      <header className="surface-flat overflow-hidden">
        <div className="flex flex-col gap-4 p-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex min-w-0 items-start gap-3">
            <Link href={backHref} className="button-secondary button-sm px-2" aria-label="Back to opportunities"><ArrowLeft className="h-4 w-4" /></Link>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="status-line"><span className={`status-dot ${lead.priority === 'urgent' ? 'status-dot-danger' : lead.priority === 'high' ? 'status-dot-warning' : ''}`} /><span className="capitalize">{lead.priority}</span></span>
              </div>
              ...
              <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-zinc-500">
                ...
                <span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3" /> {lead.destination || 'Destination not set'}</span>
              </div>
            </div>
          </div>
        ...
        <nav className="flex gap-1 overflow-x-auto border-t border-line px-3 pt-2" aria-label="Opportunity workspace sections">
          {TABS.map((tab) => (
            <button key={tab.id} type="button" aria-current={activeTab === tab.id ? 'page' : undefined} onClick={() => setActiveTab(tab.id)} className={`min-h-[48px] whitespace-nowrap border-b-2 px-3 text-left focus-visible:ring-1 focus-visible:ring-zinc-950 ${activeTab === tab.id ? 'border-zinc-950 text-zinc-950' : 'border-transparent text-zinc-500 hover:text-zinc-900'}`}>
              <span className="block text-xs font-semibold">{tab.label}</span>
              <span className="hidden text-[10px] font-normal text-zinc-400 sm:block">{tab.description}</span>
            </button>
          ))}
        </nav>
      </header>
      ...
      <div className="grid min-w-0 grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_17rem]">
        <main className="min-w-0 surface-flat p-4 sm:p-5">
          {activeTab === 'overview' && (
            <div className="space-y-6">
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                ...
                <section aria-labelledby="opportunity-heading" className="border-t border-line pt-5 lg:border-l lg:border-t-0 lg:pl-6 lg:pt-0">
                  <div id="opportunity-heading" className="section-heading">Opportunity</div>
                  <div className="mt-3 grid grid-cols-2 gap-4">
                    ...
                    <SummaryField label="Travelers" value={`${lead.pax_adults + lead.pax_children + lead.pax_infants}`} mono />
              ...
              <section aria-labelledby="qualification-heading" className="border-t border-line pt-5">
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div><div id="qualification-heading" className="section-heading">Qualification</div><div className="section-description">Only collect what is needed to progress this opportunity.</div></div>
              ...
              <section aria-labelledby="progress-heading" className="border-t border-line pt-5">
                <div id="progress-heading" className="section-heading">Progress the opportunity</div>
                <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <button type="button" onClick={() => setActiveTab('commercial')} className="surface-flat min-h-[104px] p-4 text-left hover:border-zinc-400 hover:bg-zinc-50"><div className="flex items-center gap-2 text-xs font-semibold text-zinc-900"><FileText className="h-4 w-4 text-zinc-400" /> Commercial</div><div className="mt-2 text-sm font-semibold text-zinc-900">{lead.latest_quote ? lead.latest_quote.quote_number : 'Quote not created'}</div><div className="mt-1 text-xs text-zinc-500">Quote, itinerary{showPayments ? ', payments and balance' : ''}</div></button>
                  <button type="button" onClick={() => setActiveTab('operations')} className="surface-flat min-h-[104px] p-4 text-left hover:border-zinc-400 hover:bg-zinc-50"><div className="flex items-center gap-2 text-xs font-semibold text-zinc-900"><ShieldCheck className="h-4 w-4 text-zinc-400" /> Operations</div><div className="mt-2 text-sm font-semibold text-zinc-900">{documents.length} documents · {travelers.length} travelers</div><div className="mt-1 text-xs text-zinc-500">{showDelivery ? `${TRIP_LABELS[tripStatus]} · post-sale delivery active` : 'People and documents now; delivery after Won'}</div></button>
                </div>
              </section>

              <section aria-labelledby="recent-heading" className="border-t border-line pt-5">
                <div className="flex items-center justify-between gap-3"><div><div id="recent-heading" className="section-heading">Recent activity</div><div className="section-description">Latest customer and team context</div></div><button type="button" onClick={() => setActiveTab('activity')} className="button-secondary button-sm min-h-[44px]">View activity</button></div>
              ...
              <details className="border-t border-line pt-5">
                <summary className="cursor-pointer text-xs font-semibold text-zinc-700 hover:text-zinc-950">All opportunity details</summary>
                <div className="mt-4"><AdvancedLeadDetails lead={lead} formatCurrency={formatCurrency} /></div>
              </details>
            </div>
          )}
          ...
          {activeTab === 'activity' && (
            <div className="space-y-5">
              <div>
                ...
                <div className="section-description">Customer conversations and internal context in one timeline.</div>
              </div>
              <form onSubmit={saveNote} className="rounded-lg border border-zinc-200 bg-zinc-50 p-3">
                <label htmlFor="opportunity-note" className="text-xs font-semibold text-zinc-700">Internal note</label>
          ...
          {activeTab === 'commercial' && (
            <div className="space-y-6">
              <section>
                <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="section-heading">Quote & proposal</div><div className="section-description">Commercial terms stay together with the itinerary.</div></div><button type="button" onClick={() => setQuoteOpen(true)} className="button-primary button-sm min-h-[44px]">{lead.latest_quote ? 'Edit quote' : 'Create quote'}</button></div>
                {lead.latest_quote ? <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4"><SummaryField label="Quote" value={lead.latest_quote.quote_number} mono /><SummaryField label="Selling price" value={formatCurrency(lead.latest_quote.total_selling_price)} mono /><SummaryField label="Gross profit" value={formatCurrency(lead.latest_quote.gross_profit)} mono /><SummaryField label="Margin" value={`${lead.latest_quote.profit_margin_pct}%`} mono /></div> : <div className="mt-4 rounded-lg border border-dashed border-zinc-200 p-5 text-sm text-zinc-500">No quote yet. Create one when the opportunity is qualified enough to price.</div>}
              </section>
              ...
              {showPayments && (
                <section className="border-t border-line pt-5">
                  ...
                  <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3"><div className="metric"><div className="metric-label">Package value</div><div className="metric-value">{formatCurrency(packageValue)}</div></div><div className="metric"><div className="metric-label">Paid</div><div className="metric-value">{formatCurrency(totalPaid)}</div></div><div className="metric"><div className="metric-label">Remaining</div><div className="metric-value">{formatCurrency(balance)}</div></div></div>
                  <div className="mt-5 grid gap-5 lg:grid-cols-2">
                    <div><div className="text-xs font-semibold text-zinc-800">Milestones</div><div className="mt-2 divide-y divide-zinc-100 rounded-lg border border-zinc-100">{milestones.length === 0 ? <div className="p-4 text-xs text-zinc-500">No payment milestones created.</div> : milestones.map((milestone) => <div key={milestone.id} className="flex items-center justify-between gap-4 p-3"><div><div className="text-xs font-semibold text-zinc-800">{milestone.title}</div><div className="mt-0.5 text-[11px] text-zinc-500">Due {milestone.due_date} · {milestone.percentage}%</div></div><div className="text-right"><div className="font-mono text-xs font-semibold text-zinc-800">{formatCurrency(milestone.amount)}</div><div className="text-[10px] capitalize text-zinc-500">{milestone.status}</div></div></div>)}</div></div>
                    <div><div className="text-xs font-semibold text-zinc-800">Payment history</div><div className="mt-2 divide-y divide-zinc-100 rounded-lg border border-zinc-100">{(lead.payment_records || []).length === 0 ? <div className="p-4 text-xs text-zinc-500">No payments recorded.</div> : (lead.payment_records || []).map((payment) => <div key={payment.id} className="flex items-center justify-between gap-4 p-3"><div><div className="font-mono text-xs font-semibold text-emerald-700">{formatCurrency(payment.amount)}</div><div className="mt-0.5 text-[11px] capitalize text-zinc-500">{payment.receipt_number} · {payment.method.replaceAll('_', ' ')}</div></div><div className="font-mono text-[10px] text-zinc-400">{formatAppDate(payment.received_at || payment.created_at)}</div></div>)}</div></div>
                  </div>
                </section>
              )}
            </div>
          ...
          {activeTab === 'operations' && (
            <div className="space-y-6">
              <section>
                <div className="section-heading">People & documents</div><div className="section-description">Identity and readiness information attached to this opportunity.</div>
                <div className="mt-4 grid gap-3 lg:grid-cols-2">
                  {travelers.length === 0 ? <div className="col-span-full rounded-lg border border-dashed border-zinc-200 p-5 text-sm text-zinc-500">No traveler profiles have been added.</div> : travelers.map((traveler) => (
                ...
                <div className="mt-5"><div className="text-xs font-semibold text-zinc-800">Documents</div><div className="mt-2 divide-y divide-zinc-100 rounded-lg border border-zinc-100">{documents.length === 0 ? <div className="p-4 text-xs text-zinc-500">No documents uploaded.</div> : documents.map((document) => <div key={document.id} className="flex items-center justify-between gap-4 p-3"><div><div className="text-xs font-semibold text-zinc-800">{document.title}</div><div className="mt-0.5 text-[11px] capitalize text-zinc-500">{document.category.replaceAll('_', ' ')} · {document.file_name}</div></div><div className="font-mono text-[10px] text-zinc-400">{formatAppDate(document.uploaded_at)}</div></div>)}</div></div>
              </section>

              {showDelivery ? (
                <section className="border-t border-line pt-5">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="section-heading">Post-sale delivery</div><div className="section-description">Operational execution becomes prominent after the opportunity is won.</div></div><label className="text-xs font-semibold text-zinc-600"><span className="sr-only">Trip status</span><select value={tripStatus} onChange={(event) => updateTripStatus(lead.id, event.target.value as TripLifecycleStatus)} className="select-field min-w-44">{Object.entries(TRIP_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div>
                  <div className="mt-5"><div className="text-xs font-semibold text-zinc-800">Pre-departure readiness</div><div className="mt-3 flex items-center gap-3"><div className="h-2 flex-1 overflow-hidden rounded-full bg-zinc-100"><div className="h-full bg-zinc-950" style={{ width: `${checklistTotal ? Math.round((checklistDone / checklistTotal) * 100) : 0}%` }} /></div><span className="font-mono text-[11px] font-semibold text-zinc-600">{checklistDone}/{checklistTotal || 0}</span></div></div>
                  <div className="mt-5"><div className="text-xs font-semibold text-zinc-800">Suppliers</div><div className="mt-2 divide-y divide-zinc-100 rounded-lg border border-zinc-100">{suppliers.length === 0 ? <div className="p-4 text-xs text-zinc-500">No supplier payables have been added.</div> : suppliers.map((supplier) => <div key={supplier.id} className="p-3"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-semibold text-zinc-900">{supplier.supplier_name}</div><div className="mt-0.5 text-[11px] text-zinc-500">{supplier.service_description}</div></div><span className="rounded bg-zinc-100 px-1.5 py-0.5 text-[10px] capitalize text-zinc-600">{supplier.status.replaceAll('_', ' ')}</span></div><div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 font-mono text-[11px] text-zinc-600"><span>Payable {formatCurrency(supplier.amount_payable)}</span><span>Paid {formatCurrency(supplier.amount_paid)}</span>{supplier.due_date && <span>Due {supplier.due_date}</span>}{supplier.confirmation_voucher_no && <span>Voucher {supplier.confirmation_voucher_no}</span>}</div></div>)}</div></div>
                </section>
              ) : (
                <section className="border-t border-line pt-5"><div className="rounded-lg border border-dashed border-zinc-200 bg-zinc-50 p-5"><div className="text-sm font-semibold text-zinc-900">Post-sale operations are intentionally hidden</div><p className="mt-1 text-xs leading-5 text-zinc-500">When this opportunity is marked Won, trip status, readiness and supplier execution will become available here.</p></div></section>
              )}
            </div>
          )}
        </main>
        ...
        <aside className="space-y-3 xl:sticky xl:top-0 xl:self-start">
          <section className="surface-flat p-4">
            ...
            <div className="mt-3 space-y-3">
              <div><SlaBadge lead={lead} detailed /></div>
          ...
          {omniLead.source_campaign && <section className="surface-flat p-4"><div className="section-heading">Source</div><div className="mt-2 text-xs text-zinc-600"><span className={`mr-2 inline-block h-2 w-2 rounded-full ${channelDot}`} />{sourceChannel}</div><div className="mt-2 break-words text-[11px] text-zinc-500">{omniLead.source_campaign}</div></section>}
        </aside>
      </div>
