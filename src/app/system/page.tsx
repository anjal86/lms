...
export default function SystemConsolePage() {
  ...
  if (forbidden) {
    return <div className="mx-auto max-w-xl px-6 py-20 text-center"><ShieldCheck className="mx-auto h-7 w-7 text-zinc-400" /><h1 className="mt-3 text-lg font-semibold">System access required</h1><p className="mt-1 text-sm text-zinc-500">Workspace administrator access does not grant installation-wide platform privileges.</p><Link href="/dashboard" className="button-secondary mt-5 inline-flex"><ArrowLeft className="h-4 w-4" /> Back to workspace</Link></div>;
  }

  return (
    <div className="app-page mx-auto max-w-6xl space-y-5">
      <header className="page-header">
        <div><p className="page-eyebrow">Platform administration</p><h1 className="page-title">System Console</h1><p className="page-description">Installation-wide health, queues and security configuration state. This plane is separate from workspace administration.</p></div>
        <div className="page-actions"><Link href="/dashboard" className="button-secondary"><ArrowLeft className="h-4 w-4" /> Workspace</Link><button type="button" onClick={() => void load()} className="button-secondary" disabled={loading}><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /> Refresh</button></div>
      </header>
      ...
      {loading && !health ? <div className="surface-flat flex items-center justify-center gap-2 py-20 text-sm text-zinc-500"><Loader2 className="h-4 w-4 animate-spin" /> Loading system health…</div> : error ? <div className="rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-800">{error}</div> : health && <>
        ...
        <div className="rounded-lg border border-zinc-200 bg-zinc-50 px-4 py-3 text-xs leading-5 text-zinc-600"><strong className="text-zinc-800">System boundary:</strong> this console is installation-wide and read-only. Workspace owners/admins do not inherit platform privileges. Platform administrators are provisioned separately with service-role tooling.</div>
      </>}
    </div>
  );
}
