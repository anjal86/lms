...
export default function DashboardPage() {
  ...
  return (
    <div className="workspace-page">
      <div className="workspace-header">
        <div>
          ...
          <p className="workspace-description">
            {isAgent ? 'Start with replies and follow-ups. Everything else can wait.' : 'The most important items for the team, in one place.'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Link href={isAgent ? '/my-work' : '/leads'} className="button-primary">
            {isAgent ? 'Open My Work' : 'Open all leads'}
          </Link>
          <button type="button" onClick={() => void loadSummary()} disabled={loading} className="button-secondary px-3" aria-label="Refresh">
      ...
      <section>
        <div className="mb-3">
          <h2 className="text-sm font-semibold text-zinc-950">Start here</h2>
          <p className="mt-0.5 text-xs text-zinc-500">{urgentTotal} item{urgentTotal === 1 ? '' : 's'} need attention</p>
        </div>
        <div className="grid gap-3 lg:grid-cols-3">
          ...
          <PriorityMetric title="Follow-ups due" value={summary.overdue_followups} hint="People you need to contact again" href="/follow-ups" icon={CalendarClock} tone="amber" />
          {isAgent ? (
      ...
      <div className={`grid gap-4 ${isManagement ? 'xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,0.75fr)]' : ''}`}>
        <section className="panel overflow-hidden">
          <div className="flex items-start justify-between gap-4 border-b border-zinc-100 px-5 py-4">
            <div>
              <h2 className="text-sm font-semibold text-zinc-950">What to do next</h2>
              <p className="mt-1 text-xs text-zinc-500">Work from top to bottom. The most urgent items are first.</p>
            </div>
          ...
          {loading && summary.intervention_queue.length === 0 ? (
            ...
            <div className="px-5 py-14 text-center">
              <div className="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-emerald-50 text-emerald-600">✓</div>
              <p className="mt-3 text-sm font-semibold text-zinc-900">You’re caught up</p>
              <p className="mt-1 text-xs text-zinc-500">There’s nothing urgent right now.</p>
            </div>
          ) : (
              ...
              {summary.intervention_queue.map((item, index) => (
                <Link key={item.key} href={isAgent ? '/my-work' : item.href} className="group flex items-center gap-4 px-5 py-4 transition hover:bg-zinc-50/70">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-semibold text-zinc-900">{cleanQueueTitle(item.title)}</p>
        ...
        {isManagement && (
          <section className="panel overflow-hidden">
            <div className="border-b border-zinc-100 px-5 py-4">
              <h2 className="text-sm font-semibold text-zinc-950">Also check</h2>
              <p className="mt-1 text-xs text-zinc-500">Important items that may need attention soon.</p>
            </div>
