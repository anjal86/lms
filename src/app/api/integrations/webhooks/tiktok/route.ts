...
function fieldValue(entry: Record<string, unknown>, names: string[]) {
  ...
  const candidates = [entry.field_data, entry.fields, entry.data];
  for (const candidate of candidates) {
    if (Array.isArray(candidate)) {
      for (const item of candidate) {
        if (!item || typeof item !== 'object') continue;
        const field = item as Record<string, unknown>;
        const name = String(field.name || field.field_name || field.key || '').toLowerCase();
  ...
  for (const entry of entries) {
    ...
    try {
      await ingestNormalizedLead({
        ...
        travelDates: fieldValue(entry, ['travel_dates', 'travel_date', 'departure_date']),
        ...
        metadata: { advertiser_id: advertiserId, payload: entry, request_id: payload.request_id || null, log_id: payload.log_id || null },
      });
      processed += 1;
    } catch (error) {
