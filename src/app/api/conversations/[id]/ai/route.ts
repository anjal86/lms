...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (parsed.data.action === 'resume') {
      ...
      if (error) {
        console.error('Unable to resume AI state:', error);
        return NextResponse.json({ error: 'Unable to resume AI.' }, { status: 500 });
      }
      ...
      const patch: Record<string, unknown> = {
        ...
        workspace_id: actor.profile.workspace_id,
        agent_id: agentId,
        state: parsed.data.action === 'takeover' ? 'handed_off' : 'paused',
      ...
      if (error) {
        console.error('Unable to pause or hand off AI state:', error);
        return NextResponse.json({ error: parsed.data.action === 'takeover' ? 'Unable to take over AI.' : 'Unable to pause AI.' }, { status: 500 });
      }
