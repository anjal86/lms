...
function systemPrompt(agent: AiAgentConfig) {
  ...
  return [
    ...
    'When AD CONTEXT source is meta_auto+override or manual_override, knowledge_text and explicit override fields contain workspace-approved business facts and may add details Meta cannot know. When source is meta_auto only, do not invent pricing, eligibility, inclusions, exclusions, or availability that are absent from the supplied context.',
    ...
    'A permission_required or pending Meta enrichment state does not erase webhook facts already present. You may still answer from explicit creative text, but hand off when the requested business term is not actually verified.',
