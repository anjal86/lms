...
function ChannelBadge({ lead }: { lead: Lead }) {
  ...
  const tone = CHANNEL_TONES[channel] || CHANNEL_TONES.legacy;
  return <span className={`inline-flex rounded-md border px-1.5 py-0.5 text-[9px] font-semibold ${tone}`}>{label}</span>;
}
    ...
    <div className="app-page">
      <header className="page-header">
        <div>
          ...
          <p className="page-description">Every inquiry lands here — social, messaging, email, website and manual leads — with one owner and one next action.</p>
        </div>
        <div className="page-actions">
          {(currentUser.role === 'admin' || currentUser.role === 'manager') && (
            <Link href="/connections" className="button-secondary button-sm">Connections</Link>
          )}
      ...
      <section className="metric-grid" aria-label="Pipeline summary">
        <div className="metric"><div className="metric-label">Open leads</div><div className="metric-value">{summary.visible_count}</div><div className="metric-hint">Visible to your team</div></div>
        <div className="metric"><div className="metric-label">Needs reply</div><div className="metric-value">{summary.pending_sla_count}</div><div className="metric-hint">New inquiries waiting for first contact</div></div>
        <div className="metric"><div className="metric-label">Overdue</div><div className="metric-value">{summary.overdue_count}</div><div className="metric-hint">Follow-ups that need attention</div></div>
        <div className="metric"><div className="metric-label">Won value</div><div className="metric-value">{formatCurrency(summary.won_value)}</div><div className="metric-hint">Across {summary.won_count} won leads</div></div>
      </section>
      ...
      <section className="surface-flat overflow-visible">
            ...
            {tabs.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`min-h-9 whitespace-nowrap border-b-2 px-3 text-xs font-semibold transition ${active ? 'border-blue-600 text-blue-700' : 'border-transparent text-zinc-500 hover:text-zinc-900'}`}
                >
                  {item.label}<span className="ml-1.5 font-mono text-[10px] font-medium text-zinc-400">{item.count}</span>
                </button>
              );
            })}
          </nav>
            ...
            {filtersOpen && (
              <div className="absolute right-0 z-30 mt-2 w-[min(22rem,calc(100vw-2rem))] rounded-lg border border-zinc-200 bg-white p-4 shadow-lg">
                ...
                <div className="space-y-3">
                  <label className="block text-xs font-semibold text-zinc-700">Channel
                    <select value={channel} onChange={(event) => updateParams({ channel: event.target.value, page: null })} className="select-field mt-1.5">
                      ...
                      <option value="legacy">Other / older leads</option>
                    </select>
                  </label>
                  <label className="block text-xs font-semibold text-zinc-700">Destination
                    <select value={destination} onChange={(event) => updateParams({ dest: event.target.value, page: null })} className="select-field mt-1.5">
                      <option value="ALL">All destinations</option>
                      {destinations.map((item) => <option key={item} value={item}>{item}</option>)}
                    </select>
                  </label>
                  <label className="block text-xs font-semibold text-zinc-700">Trip status
                    <select value={tripStatus} onChange={(event) => updateParams({ trip: event.target.value, page: null })} className="select-field mt-1.5">
                      <option value="ALL">All trip statuses</option>
                      <option value="planning">Planning</option><option value="booked">Booked</option><option value="pre_departure">Pre-departure</option><option value="on_trip">On trip</option><option value="completed">Completed</option>
                    </select>
                  </label>
        ...
        {activeFilterCount > 0 && (
          <div className="flex flex-wrap items-center gap-2 border-b border-zinc-200 px-3 py-2 sm:px-4">
            ...
            {tripStatus !== 'ALL' && <button type="button" onClick={() => updateParams({ trip: null })} className="button-ghost button-sm">Trip: {TRIP_STATUS_LABELS[tripStatus] || tripStatus} <X className="h-3 w-3" /></button>}
          </div>
        )}

        {error ? (
          <div className="empty-state" role="alert"><AlertCircle className="h-5 w-5 text-danger" /><h2 className="empty-state-title mt-3">Leads could not be loaded</h2><p className="empty-state-description">{error} Check the database connection, then try again.</p><button type="button" onClick={refresh} className="button-primary mt-4">Try again</button></div>
                      ...
                      <tr key={lead.id} className={selectedIds.includes(lead.id) ? 'bg-blue-50/50' : undefined}>
                        ...
                        <td>
                          <Link href={`/leads/${lead.id}/workspace`} className="group block min-w-[12rem]">
                            <div className="flex items-center gap-2"><div className="font-semibold text-zinc-950 group-hover:underline">{lead.customer_name}</div><ChannelBadge lead={lead} /></div>
                            <div className="mt-0.5 flex items-center gap-2 font-mono text-[11px] text-zinc-500"><span>{lead.lead_code}</span><span>{lead.customer_phone}</span></div>
              ...
              {leads.map((lead) => {
                ...
                return (
                  <article key={lead.id} className="p-4">
                    <div className="flex items-start justify-between gap-3"><Link href={`/leads/${lead.id}/workspace`} className="min-w-0 flex-1"><div className="flex items-center gap-2"><div className="truncate text-sm font-semibold text-zinc-950">{lead.customer_name}</div><ChannelBadge lead={lead} /></div><div className="mt-1 font-mono text-[11px] text-zinc-500">{lead.lead_code} · {lead.customer_phone}</div></Link><input type="checkbox" checked={selectedIds.includes(lead.id)} onChange={() => setSelectedIds((ids) => ids.includes(lead.id) ? ids.filter((id) => id !== lead.id) : [...ids, lead.id])} aria-label={`Select ${lead.customer_name}`} /></div>
                    <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3 text-xs">
                      <div><div className="text-[10px] font-semibold uppercase tracking-wide text-zinc-400">Destination</div><div className="mt-0.5 font-medium text-zinc-800">{lead.destination}</div></div>
                      <div><div className="text-[10px] font-semibold uppercase tracking-wide text-zinc-400">Owner</div><div className="mt-0.5 font-medium text-zinc-800">{owner?.full_name || 'Unassigned'}</div></div>
