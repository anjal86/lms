...
export default function GenericLeadList() {
  ...
  const { allProfiles, formatAppDate } = useApp();
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setItems((payload?.items || []) as Lead[]);
      setTotal(Number(payload?.total || 0));
      setTotalPages(Number(payload?.totalPages || 1));
      setError(null);
    } catch (loadError) {
      if ((loadError as { name?: string })?.name !== 'AbortError') setError(loadError instanceof Error ? loadError.message : 'Unable to load records.');
    } finally {
      setLoading(false);
    }
      ...
      <section className="surface-flat overflow-hidden">
        <div className="flex flex-col gap-3 border-b border-zinc-200 p-3 sm:flex-row sm:items-center sm:justify-between">
          <label className="relative block w-full max-w-md">
            ...
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${leadPlural.toLowerCase()}…`} aria-label={`Search ${leadPlural}`} className="h-9 w-full rounded-md border border-zinc-200 bg-white pl-9 pr-3 text-sm outline-none focus:border-zinc-400" />
          </label>
          <div className="text-[11px] text-zinc-500">Business columns adapt from Custom Fields</div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[880px] border-collapse text-left">
            <thead className="bg-zinc-50/80">
              <tr className="border-b border-zinc-200 text-[10px] font-semibold uppercase tracking-[0.08em] text-zinc-400">
                ...
                <tr key={lead.id} className="group bg-white hover:bg-zinc-50/70">
                  <td className="px-4 py-3">
                    <Link href={`/leads/${lead.id}/workspace`} className="block min-w-48">
                      ...
                      <div className="mt-1 flex items-center gap-2 text-[11px] text-zinc-500"><span className="font-mono">{lead.customer_phone || 'No phone'}</span>{lead.source && <><span>·</span><span className="capitalize">{lead.source.replaceAll('_', ' ')}</span></>}</div>
                    </Link>
                  </td>
                  ...
                  <td className="px-3 py-3"><span className="rounded-md border border-zinc-200 bg-zinc-50 px-2 py-1 text-[10px] font-semibold text-zinc-600">{stageNames.get(lead.pipeline_stage_id || '') || 'New'}</span></td>
                  <td className="px-3 py-3 text-xs text-zinc-600">{ownerNames.get(lead.assigned_to || '') || 'Unassigned'}</td>
                  <td className="whitespace-nowrap px-3 py-3 font-mono text-[10px] text-zinc-400">{formatAppDate(lead.updated_at || lead.created_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
