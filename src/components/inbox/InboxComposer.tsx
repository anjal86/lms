...
export default function InboxComposer({ workspaceId, conversationId, mode, onModeChange, value, onChange, canReply, loading, sending, composerRef, onTyping, onSend }: Props) {
  ...
  return <div data-chat-composer="true" className={mode === 'internal' ? 'border-amber-200 bg-amber-50/60' : ''}>
    <div data-composer-tabs="true">
      ...
      {mode === 'outbound' && !canReply && <span className="ml-auto text-[10px] font-semibold text-amber-700">Provider reply window closed</span>}
    </div>
    ...
    <div data-composer-actions="true" className="relative flex items-center justify-between gap-2 px-3 py-1.5">
      <div className="flex items-center gap-1">
        {mode === 'outbound' && <>
          ...
          <button type="button" aria-label="Emoji" title="Emoji" onClick={() => { setEmojiOpen((open) => !open); setQuickOpen(false); }} className="rounded-md p-2 text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900"><Smile className="h-4 w-4" /></button>
          <button type="button" aria-label="Saved replies" title="Saved replies" onClick={() => { setQuickOpen((open) => !open); setEmojiOpen(false); }} className="rounded-md p-2 text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900"><BookOpen className="h-4 w-4" /></button>
        </>}
        <span className="ml-1 hidden text-[10px] text-zinc-400 sm:inline">Enter to send · Shift+Enter new line</span>
      </div>
      <button type="button" onClick={() => void send()} disabled={Boolean(sending || loading || uploading || (!value.trim() && !attachment) || (mode === 'outbound' && !canReply))} className={`inline-flex min-h-8 items-center gap-1.5 rounded-md px-3 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40 ${mode === 'internal' ? 'bg-amber-600 hover:bg-amber-700' : 'bg-[#0866ff] hover:bg-[#075ee5]'}`}>{sending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Send className="h-3.5 w-3.5" />}{mode === 'internal' ? 'Add note' : 'Send'}</button>

      {emojiOpen && <div className="absolute bottom-11 left-10 z-40 grid w-52 grid-cols-4 gap-1 rounded-xl border border-zinc-200 bg-white p-2 shadow-xl">{EMOJIS.map((emoji) => <button key={emoji} type="button" onClick={() => { onChange(`${value}${emoji}`); setEmojiOpen(false); composerRef?.current?.focus(); }} className="rounded-lg p-2 text-lg hover:bg-zinc-100">{emoji}</button>)}</div>}

      {quickOpen && <div className="absolute bottom-11 left-16 z-40 w-72 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-xl">
