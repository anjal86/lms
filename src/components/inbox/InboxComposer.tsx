...
export default function InboxComposer({ workspaceId, conversationId, mode, onModeChange, value, onChange, canReply, loading, sending, composerRef, onTyping, onSend }: Props) {
  ...
  return <div data-chat-composer="true" className={mode === 'internal' ? 'border-amber-200 bg-amber-50/60' : 'bg-white'}>
    ...
    <textarea
      ...
      className="max-h-48 min-h-24 w-full resize-y bg-transparent px-3.5 py-3 text-sm leading-6 outline-none placeholder:text-zinc-400 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500/20 disabled:bg-zinc-50"
    />
    ...
    <div data-composer-actions="true" className="relative flex items-center justify-between gap-2 px-3 py-2">
      <div className="flex items-center gap-1">
        {mode === 'outbound' && <>
          ...
          <button type="button" aria-label="Attach file" title="Attach file" onClick={() => fileRef.current?.click()} disabled={uploading || disabled} className="flex min-h-10 min-w-10 items-center justify-center rounded-lg text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30 disabled:opacity-40">{uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Paperclip className="h-4 w-4" />}</button>
          <button type="button" aria-label="Emoji" title="Emoji" aria-expanded={emojiOpen} onClick={() => { setEmojiOpen((open) => !open); setQuickOpen(false); }} disabled={disabled} className="flex min-h-10 min-w-10 items-center justify-center rounded-lg text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30 disabled:opacity-40"><Smile className="h-4 w-4" /></button>
          <button type="button" aria-label="Saved replies" title="Saved replies" aria-expanded={quickOpen} onClick={() => { setQuickOpen((open) => !open); setEmojiOpen(false); }} disabled={disabled} className="flex min-h-10 min-w-10 items-center justify-center rounded-lg text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30 disabled:opacity-40"><BookOpen className="h-4 w-4" /></button>
        </>}
        <span className="ml-1 hidden text-[10px] text-zinc-400 sm:inline">Enter to send · Shift+Enter for a new line</span>
      </div>
      <button type="button" onClick={() => void send()} disabled={Boolean(sending || loading || uploading || (!value.trim() && !attachment) || (mode === 'outbound' && !canReply))} className={`inline-flex min-h-10 items-center gap-1.5 rounded-lg px-4 text-xs font-semibold text-white transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-40 ${mode === 'internal' ? 'bg-amber-600 hover:bg-amber-700 focus-visible:ring-amber-500/40' : 'bg-blue-600 hover:bg-blue-700 focus-visible:ring-blue-500/40'}`}>{sending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Send className="h-3.5 w-3.5" />}{mode === 'internal' ? 'Add note' : 'Send'}</button>

      {emojiOpen && <div role="dialog" aria-label="Choose emoji" className="absolute bottom-12 left-10 z-40 grid w-52 grid-cols-4 gap-1 rounded-xl border border-zinc-200 bg-white p-2 shadow-xl">{EMOJIS.map((emoji) => <button key={emoji} type="button" onClick={() => { onChange(`${value}${emoji}`); setEmojiOpen(false); composerRef?.current?.focus(); }} className="flex min-h-10 min-w-10 items-center justify-center rounded-lg text-lg hover:bg-zinc-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30">{emoji}</button>)}</div>}

      {quickOpen && <div role="dialog" aria-label="Saved replies" className="absolute bottom-12 left-16 z-40 w-72 overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-zinc-100 px-3 py-2.5"><div className="text-xs font-semibold">Saved replies</div>{value.trim() && <button type="button" onClick={addQuickReply} className="inline-flex min-h-9 items-center gap-1 rounded-md px-1.5 text-[10px] font-medium text-blue-600 hover:bg-blue-50 hover:text-blue-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/30"><Plus className="h-3 w-3" /> Save current</button>}</div>
