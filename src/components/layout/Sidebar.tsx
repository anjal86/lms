      ...
      {items.map((item) => {
        ...
        return (
          <Link
            ...
            className={`group flex items-center transition-all ${collapsed ? 'mx-auto h-10 w-10 justify-center rounded-xl' : 'min-h-11 gap-3 rounded-xl px-3 text-[13px] font-semibold'} ${active ? `${ACTIVE[item.tone]} shadow-sm` : 'text-slate-400 hover:bg-white/[0.06] hover:text-slate-100'}`}
          >
...
export default function Sidebar() {
  ...
  const admin: NavItem[] = isAgent ? [] : [
    ...
    ...(inboxEnabled && can('automations.view') ? [{ label: 'Automations', href: '/settings/automations', icon: Zap, tone: 'violet' as const }] : []),
    ...
    { label: 'Service Levels', href: '/settings/service-levels', icon: Sparkles, tone: 'amber' },
  ...
  return (
    <aside className={`hidden shrink-0 flex-col border-r border-slate-800/80 bg-[linear-gradient(180deg,#081226_0%,#0b1730_48%,#10172a_100%)] text-slate-300 transition-[width] duration-200 md:flex ${collapsed ? 'w-16' : 'w-60'}`}>
      ...
      <nav className={`flex-1 overflow-y-auto py-2 ${collapsed ? 'px-2' : 'px-2.5'}`}>
        <Group label={isAgent ? 'My day' : 'Work'} items={work} pathname={pathname} collapsed={collapsed} />
        <Group label="Reports" items={reports} pathname={pathname} collapsed={collapsed} />
        <Group label="Admin" items={admin} pathname={pathname} collapsed={collapsed} />
      </nav>

      <div className={`border-t border-white/[0.07] ${collapsed ? 'p-2' : 'p-3'}`}>
        <Link href="/profile" title={collapsed ? `${currentUser.full_name} · ${currentUser.role}` : undefined} className={`block rounded-xl bg-white/[0.04] hover:bg-white/[0.07] ${collapsed ? 'p-1' : 'p-3'}`}>
          <div className="flex items-center gap-2.5">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-cyan-400 to-blue-500 text-[10px] font-bold text-white">{currentUser.full_name.split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]?.toUpperCase()).join('')}</span>
            {!collapsed && <div className="min-w-0 flex-1"><div className="truncate text-[13px] font-semibold text-slate-100">{currentUser.full_name}</div><div className="text-[10px] uppercase tracking-wide text-slate-500">{currentUser.role}</div></div>}
          </div>
        </Link>
