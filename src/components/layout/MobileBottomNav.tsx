...
export default function MobileBottomNav() {
  ...
  const primaryItems: MobileNavItem[] = [
    ...
    ...(isAgent && inboxEnabled && can('contacts.view') ? [{ label: contactPlural, href: '/contacts', icon: Users }] : []),
  ];
  ...
  return (
    <>
      {moreOpen && !isAgent && (
        <div className="fixed inset-0 z-50 bg-zinc-950/25 backdrop-blur-[2px] md:hidden" onClick={() => setMoreOpen(false)}>
          <section role="dialog" aria-modal="true" aria-label="More navigation" className="absolute inset-x-0 bottom-0 rounded-t-2xl border-t border-zinc-200 bg-white px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="flex items-center justify-between gap-3">
              <div><div className="text-sm font-semibold text-zinc-950">More</div><div className="mt-0.5 text-[11px] text-zinc-500">Workspace management and configuration</div></div>
              <button type="button" onClick={() => setMoreOpen(false)} className="flex h-10 w-10 items-center justify-center rounded-xl text-zinc-500 hover:bg-zinc-100 hover:text-zinc-950" aria-label="Close more menu"><X className="h-4 w-4" /></button>
            </div>
            <div className="mt-3 grid gap-1.5 sm:grid-cols-2">
              {moreItems.map((item) => {
                const Icon = item.icon;
                const active = pathActive(pathname, item.href);
                return <Link key={item.href} href={item.href} className={`flex min-h-14 items-center gap-3 rounded-xl border px-3 py-2.5 transition ${active ? 'border-blue-200 bg-blue-50 text-blue-800' : 'border-zinc-200 bg-white text-zinc-900 hover:bg-zinc-50'}`}><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg ${active ? 'bg-blue-100 text-blue-700' : 'bg-zinc-100 text-zinc-600'}`}><Icon className="h-4 w-4" /></span><span className="min-w-0"><span className="block text-xs font-semibold">{item.label}</span><span className="mt-0.5 block truncate text-[10px] text-zinc-500">{item.description}</span></span></Link>;
              })}
            </div>
          </section>
        </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-zinc-200 bg-white/95 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-1.5 backdrop-blur md:hidden" aria-label="Primary navigation">
        <div className="mx-auto grid max-w-lg gap-1" style={{ gridTemplateColumns: `repeat(${Math.max(1, navCount)}, minmax(0, 1fr))` }}>
          {primaryItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-xl px-2 text-[10px] font-medium transition-colors ${isActive ? 'bg-blue-50 text-blue-700' : 'text-zinc-500 hover:bg-zinc-50 hover:text-zinc-900'}`}
              >
          ...
          {!isAgent && (
            <button type="button" onClick={() => setMoreOpen(true)} aria-expanded={moreOpen} aria-haspopup="dialog" className={`flex min-h-[52px] flex-col items-center justify-center gap-1 rounded-xl px-2 text-[10px] font-medium transition-colors ${moreActive || moreOpen ? 'bg-blue-50 text-blue-700' : 'text-zinc-500 hover:bg-zinc-50 hover:text-zinc-900'}`}>
              <Ellipsis className={`h-[18px] w-[18px] ${moreActive || moreOpen ? 'text-blue-600' : 'text-zinc-500'}`} />
              <span>More</span>
            </button>
          )}
        </div>
