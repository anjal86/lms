...
export default function Header() {
  ...
  return (
    <>
      <header className="sticky top-0 z-30 flex items-center justify-between border-b border-zinc-200 bg-white/95 px-3 backdrop-blur md:px-4">
        <div className="flex min-w-0 items-center gap-2.5">
          <button
            ...
            className="inline-flex h-9 w-9 items-center justify-center rounded-lg text-zinc-500 transition hover:bg-zinc-100 hover:text-zinc-950 md:hidden"
          >
          ...
          <button
            ...
            className="flex h-9 items-center gap-2 rounded-lg border border-zinc-200 bg-zinc-50/80 px-3 text-[13px] text-zinc-500 transition hover:border-zinc-300 hover:bg-white sm:w-64 md:w-80"
          >
            <Search className="h-3.5 w-3.5 shrink-0" />
            <span className="hidden truncate sm:inline">Search workspace</span>
            <kbd className="ml-auto hidden rounded-md border border-zinc-200 bg-white px-1.5 py-0.5 text-[10px] font-medium text-zinc-400 lg:inline">⌘K</kbd>
          </button>
        </div>

        <div className="flex items-center gap-1.5">
          {leadsEnabled && canManage && isTravel && (
            <button type="button" onClick={() => setIsCsvModalOpen(true)} className="button-secondary button-sm hidden sm:inline-flex" title="Import CSV">
              <FileSpreadsheet className="h-3.5 w-3.5" /> Import
            </button>
          )}

          {leadsEnabled && (
            <button type="button" onClick={() => setIsLeadModalOpen(true)} className="button-primary button-sm">
              <Plus className="h-3.5 w-3.5" />
              <span className="hidden sm:inline">New {leadLabel}</span>
          ...
          <div className="relative" data-notif-dropdown="true">
            <button
              ...
              className="relative inline-flex h-9 w-9 items-center justify-center rounded-lg text-zinc-500 transition hover:bg-zinc-100 hover:text-zinc-950"
            >
              <Bell className="h-4 w-4" />
              {unreadCount > 0 && <span className="absolute right-2 top-2 h-1.5 w-1.5 rounded-full bg-blue-600 ring-2 ring-white" />}
            </button>

            {isNotifOpen && (
              <div className="absolute right-0 top-[calc(100%+8px)] z-50 w-[min(23rem,calc(100vw-1rem))] overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-xl animate-in fade-in-0 zoom-in-95 duration-100">
                <div className="flex items-center justify-between border-b border-zinc-100 px-4 py-3">
                  <span className="text-[13px] font-semibold text-zinc-950">Notifications</span>
                  {unreadCount > 0 && (
                    <button type="button" onClick={markAllNotificationsAsRead} className="text-[12px] font-medium text-blue-600 hover:text-blue-700">Mark all read</button>
                  )}
                </div>
                <div className="max-h-80 divide-y divide-zinc-100 overflow-y-auto">
                  {notifications.length === 0 ? (
                    ...
                    <button
                      ...
                      className={`flex w-full items-start gap-3 px-4 py-3 text-left transition hover:bg-zinc-50 ${!n.is_read ? 'bg-blue-50/35' : ''}`}
                    >
                      {n.type === 'sla_breach'
      ...
      {isMobileNavOpen && (
        <div className="fixed inset-0 z-50 flex bg-zinc-950/25 backdrop-blur-[2px] md:hidden" role="dialog" aria-modal="true" aria-label="Navigation menu">
          <button type="button" className="fixed inset-0" onClick={() => setIsMobileNavOpen(false)} aria-label="Close menu" />
          <div className="relative z-10 flex h-full w-72 max-w-[88vw] flex-col border-r border-zinc-200 bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-zinc-200 px-3 py-3">
              <Link href="/dashboard" onClick={() => setIsMobileNavOpen(false)} className="flex min-w-0 items-center gap-2.5">
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-blue-600 text-[11px] font-semibold text-white">{workspaceInitial}</span>
                <span className="min-w-0">
                  <span className="block truncate text-[13px] font-semibold text-zinc-950">{workspaceName}</span>
                  <span className="block truncate text-[11px] text-zinc-500">{workspaceLabel}</span>
                </span>
              </Link>
              <button type="button" onClick={() => setIsMobileNavOpen(false)} className="flex h-9 w-9 items-center justify-center rounded-lg text-zinc-500 hover:bg-zinc-100 hover:text-zinc-950" aria-label="Close menu">
                <X className="h-4 w-4" />
              </button>
            </div>
              ...
              {mobileNavItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={`flex h-10 items-center gap-2.5 rounded-lg px-3 text-[13px] font-medium transition ${isActive ? 'bg-blue-50 text-blue-700' : 'text-zinc-600 hover:bg-zinc-100 hover:text-zinc-950'}`}
                  >
