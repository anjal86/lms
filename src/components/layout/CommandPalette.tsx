...
export default function CommandPalette({ isOpen, onClose, onOpenNewLead, onOpenCsv }: CommandPaletteProps) {
  ...
  const basicActions = [{ label: 'Add lead', hint: 'Create a new lead', action: () => { onClose(); onOpenNewLead?.(); }, icon: Plus }];
  const managementActions = [
    { label: 'Connect a lead source', hint: 'Open omnichannel connections', action: () => { onClose(); router.push('/connections'); }, icon: PlugZap },
    { label: 'Import leads', hint: 'Upload a CSV file', action: () => { onClose(); onOpenCsv?.(); }, icon: FileSpreadsheet },
    {
      label: 'Export leads',
      hint: 'Download visible leads as CSV',
      action: () => {
        onClose();
        exportToCsv(`wanderlust_leads_${new Date().toISOString().slice(0, 10)}`, allLeads, [
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Search leads and pages" className="fixed inset-0 z-50 flex items-start justify-center bg-zinc-950/40 p-4 pt-20 backdrop-blur-2xs animate-in fade-in duration-100" onClick={onClose}>
      <div className="flex max-h-[70vh] w-full max-w-xl flex-col overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-2xl animate-in zoom-in-95 duration-100" onClick={(event) => event.stopPropagation()}>
        <div className="flex h-12 items-center gap-2.5 border-b border-zinc-200 bg-zinc-50/50 px-4">
          ...
          <input ref={inputRef} type="text" value={query} onChange={(event) => setQuery(event.target.value)} placeholder={isAgent ? 'Search my leads or pages…' : 'Search leads, channels, team or pages…'} aria-label={isAgent ? 'Search my leads or pages' : 'Search leads, channels, team or pages'} className="flex-1 bg-transparent text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none" />
        ...
        <div className="space-y-3 overflow-y-auto p-2 text-xs">
          ...
          {matchingLeads.length > 0 && <section><div className="px-2 py-1 text-[10px] font-semibold uppercase tracking-wide text-zinc-400">Leads</div><div className="space-y-0.5">{matchingLeads.map((lead) => <button key={lead.id} onClick={() => navigate(leadPath(lead.id))} className="group flex min-h-12 w-full items-center justify-between rounded-md px-3 text-left transition hover:bg-zinc-100"><div className="min-w-0"><div className="flex items-center gap-2"><span className="truncate text-sm font-medium text-zinc-900">{lead.title}</span>{!isAgent && lead.meta.possible_duplicate === true && <span className="rounded bg-amber-50 px-1.5 py-0.5 text-[9px] font-medium text-amber-700">Possible duplicate</span>}</div><span className="mt-0.5 flex items-center gap-1 truncate text-[11px] text-zinc-500"><MapPin className="h-3 w-3" /> {lead.subtitle}</span></div>{!isAgent && <span className="ml-3 flex-none rounded border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 text-[10px] capitalize text-zinc-600">{String(lead.meta.stage || 'lead').replaceAll('_', ' ')}</span>}</button>)}</div></section>}
