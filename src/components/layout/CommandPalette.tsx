...
export default function CommandPalette({ isOpen, onClose, onOpenNewLead, onOpenCsv }: CommandPaletteProps) {
  ...
  const managementNavItems = [
    { label: 'Today', hint: 'See what needs attention', path: '/dashboard', icon: LayoutDashboard },
    { label: 'All Leads', hint: 'View and manage every visible lead', path: '/leads', icon: ArrowRight },
  ...
  const navItems = (isAgent ? agentNavItems : managementNavItems)
  ...
  const adminActions = canManageStorage
    ? [{
        label: 'Download backup',
        hint: 'Download a JSON snapshot',
        action: () => {
          onClose();
          exportCrmBackup();
        },
      ...
      >
        <div className="flex h-12 items-center gap-2.5 border-b border-zinc-200 bg-zinc-50/50 px-4">
          ...
          <input
            ...
            placeholder={isAgent ? 'Search my leads or pages…' : 'Search leads, team or pages…'}
            aria-label={isAgent ? 'Search my leads or pages' : 'Search leads, team or pages'}
            className="flex-1 bg-transparent text-sm text-zinc-900 placeholder-zinc-400 focus:outline-none"
          />
          {query && (
            <button type="button" onClick={() => setQuery('')} aria-label="Clear search" className="p-1 text-zinc-400 hover:text-zinc-600">
              <X className="h-4 w-4" />
            </button>
          )}
        ...
        <div className="space-y-3 overflow-y-auto p-2 text-xs">
              ...
              <div className="space-y-0.5">
                {actionItems.map((item) => (
                  <button key={item.label} onClick={item.action} className="group flex min-h-10 w-full items-center justify-between rounded-lg px-3 text-left text-zinc-800 transition hover:bg-zinc-100">
                    ...
                    <span className="hidden text-[11px] text-zinc-400 sm:block">{item.hint}</span>
                  </button>
                ))}
              </div>
            </section>
                  ...
                  <button key={lead.id} onClick={() => navigate(leadPath(lead.id))} className="group flex min-h-12 w-full items-center justify-between rounded-lg px-3 text-left transition hover:bg-zinc-100">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {!isAgent && lead.meta.possible_duplicate === true && <span className="rounded bg-amber-50 px-1.5 py-0.5 text-[9px] font-medium text-amber-700">Possible duplicate</span>}
                      </div>
                      <span className="mt-0.5 flex items-center gap-1 truncate text-[11px] text-zinc-500">
                        <MapPin className="h-3 w-3" /> {lead.subtitle}
                      </span>
                    </div>
                    {!isAgent && (
                      <span className="ml-3 flex-none rounded border border-zinc-200 bg-zinc-50 px-1.5 py-0.5 text-[10px] capitalize text-zinc-600">
                        {String(lead.meta.stage || 'lead').replaceAll('_', ' ')}
                      </span>
                    )}
                  </button>
                ))}
          ...
          {totalItemsCount === 0 && q.length < 2 && (
            <div className="py-8 text-center text-sm text-zinc-400">Type at least 2 characters to search.</div>
          )}
        </div>

        <div className="flex min-h-9 items-center justify-between border-t border-zinc-100 bg-zinc-50 px-3 text-[11px] text-zinc-400">
          <span>{isAgent ? 'Search only shows work you can access.' : 'Search respects your access.'}</span>
