-- Atomic manager-facing save function for workspace AI agents and channel bindings.
...
begin
  ...
  if v_agent_id is null then
    insert into public.ai_agents(
      workspace_id,name,description,provider,model,instructions,tone,languages,mode,is_active,
      ...
      v_workspace_id,btrim(p_name),nullif(btrim(coalesce(p_description,'')),''),'mistral',
...
revoke all on function public.save_workspace_ai_agent(uuid,text,text,text,text,text,text[],text,boolean,numeric,numeric,integer,integer,text,text[],boolean,uuid[]) from public,anon;
grant execute on function public.save_workspace_ai_agent(uuid,text,text,text,text,text,text[],text,boolean,numeric,numeric,integer,integer,text,text[],boolean,uuid[]) to authenticated,service_role;
