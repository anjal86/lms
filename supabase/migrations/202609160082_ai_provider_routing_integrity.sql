-- Keep provider identity immutable and ensure disabled BYOK providers never
-- reserve live conversations away from normal human routing.

begin;

create or replace function public.protect_ai_provider_identity()
returns trigger
language plpgsql
security definer
...
as $$
begin
  if old.provider is distinct from new.provider or old.api_style is distinct from new.api_style then
    raise exception 'AI provider type cannot be changed after creation; create a new provider connection instead' using errcode='22023';
  end if;
  return new;
end;
$$;

drop trigger if exists trg_protect_ai_provider_identity on public.ai_provider_configs;
create trigger trg_protect_ai_provider_identity
before update of provider,api_style on public.ai_provider_configs
for each row execute function public.protect_ai_provider_identity();
...
begin
  ...
  select exists (
    ...
    left join public.ai_provider_configs pc
      on pc.id = a.provider_config_id and pc.workspace_id = a.workspace_id
    where ac.workspace_id = v_workspace_id
      ...
      and (a.provider_config_id is null or pc.is_active = true)
  ) into v_ai_auto;
